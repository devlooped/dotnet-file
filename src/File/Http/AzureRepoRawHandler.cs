...
/// </summary>
class AzureRepoRawHandler(HttpMessageHandler inner) : DelegatingHandler(inner)
{
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        if (request.RequestUri?.Host.Equals("dev.azure.com") == true)
        {
            ...
            var paths = uri.GetComponents(UriComponents.Path, UriFormat.Unescaped).Split(['/'], StringSplitOptions.RemoveEmptyEntries);
            if (paths.Length == 4)
            {
                ...
                if (args.TryGetValue("path", out var path))
                {
                    ...
                    if (args.TryGetValue("version", out var branch))
                    {
                        builder.Append("&versionDescriptor[version]=")
                            .Append(branch.StartsWith("GB") ? branch[2..] : branch);
                    }
